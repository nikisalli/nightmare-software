...
from nightmare_pybullet.sdf.naming import *
from nightmare_pybullet.sdf.conversions import *
...
def find_mesh_in_catkin_ws(filename):
                ...
                if currfile.endswith('.stl') or currfile.endswith('.dae'):
                    ...
                    for path_part in root.split('/'):
                        ...
                        if os.path.exists(partial_path + '/package.xml'):
                            break
                    catkin_stack_path = partial_path.replace(
                        path_part + '/', '')
                    filename_path = os.path.join(
                        root, currfile).replace(catkin_stack_path, '')
    ...
    matching = [
        path for path in find_mesh_in_catkin_ws.cache if filename in path]
...
def find_model_in_gazebo_dir(modelname):
            ...
            for dirpath, dirs, files in os.walk(models_path, followlinks=True):
                ...
                for currfile in files:
                    ...
                    try:
                        ...
                        print("Error parsing SDF file %s (%s). Ignoring model and continuing." % (
                            filename_path, e))
                        continue
                    root = tree.getroot()
                    if root.tag != 'sdf':
    ...
    if '/' in modelname:  # path-based
        for models_path in models_paths + ['.']:
            modelfile_paths = glob.glob(
                os.path.join(models_path, modelname, '*.sdf'))
...
def pose2origin(node, pose):
    ...
    ET.SubElement(node, 'origin', {'xyz': array2string(
        rounded(xyz)), 'rpy': array2string(rounded(rpy))})
...
class World(object):
    ...
    def from_tree(self, node, **kwargs):
        ...
        if node.findall('world'):
            ...
            # TODO lights
        self.models += [Model(tree=model_node, version=self.version)
                        for model_node in node.findall('model')]
...
class Model(SpatialEntity):
    ...
    def __repr__(self):
        return ''.join((
            ...
            '  links:\n',
            '    %s' % '\n    '.join([indent(str(li), 4) for li in self.links]),
            '\n',
            '  joints:\n',
    ...
    def from_tree(self, node, **kwargs):
        ...
        self.links = [Link(self, tree=link_node)
                      for link_node in node.findall('link')]
        self.joints = [Joint(self, tree=joint_node)
    ...
    def get_link(self, requested_linkname, prefix=''):
        ...
        for link in self.links:
            if full_prefix + link.name == requested_linkname:
                return link
        for submodel in self.submodels:
            res = submodel.get_link(
                requested_linkname, prefix + '::' + submodel.name if prefix else submodel.name)
            if res:
                return res
    ...
    def find_root_link(self):
        if not self.links:
            print(
                'Model %s has no links and therefore no root link. Aborting' % self.name)
            return None
    ...
    def get_parent(self, requested_linkname, prefix='', visited=[]):
        ...
        for submodel in self.submodels:
            res = submodel.get_parent(
                requested_linkname, full_prefix + submodel.name, visited + [self.name])
            if res:
                return res
    ...
    def plot(self, graph, prefix=''):
        ...
        for link in self.links:
            graph.add_node(full_prefix + link.name,
                           label=full_prefix + link.name + '\\nrel: ' + homogeneous2tq_string_rounded(link.pose) + '\\nabs: ' + homogeneous2tq_string_rounded(link.pose_world))
        subgraph = graph.add_subgraph([full_prefix + link.name for link in self.links], 'cluster_' + self.name, color='gray',
...
class Link(SpatialEntity):
    ...
    def from_tree(self, node):
        ...
        self.collisions = [Collision(tree=collision_node)
                           for collision_node in node.findall('collision')]
        self.visuals = [Visual(tree=visual_node)
                        for visual_node in node.findall('visual')]
    ...
    def add_urdf_elements(self, node, prefix):
        full_prefix = prefix + '::' if prefix else ''
        linknode = ET.SubElement(
            node, 'link', {'name': sdf2tfname(full_prefix + self.name)})
        ...
        if self.tree_parent_joint:
            if self.tree_parent_joint.parent_model == self.parent_model:
                urdf_pose = concatenate_matrices(inverse_matrix(
                    self.tree_parent_joint.pose_world), self.pose_world)
...
class Joint(SpatialEntity):
    ...
    def from_tree(self, node):
        ...
        if get_node(node, 'axis2') is not None:
    ...
    def add_urdf_elements(self, node, prefix):
        ...
        parentnode = ET.SubElement(jointnode, 'parent', {
        ...
        if self.tree_parent_link.parent_model == self.parent_model:
            pose2origin(jointnode, concatenate_matrices(
                inverse_matrix(parent_pose_world), self.pose_world))
        else:  # joint crosses includes
            pose2origin(jointnode, concatenate_matrices(inverse_matrix(
                parent_pose_world), self.tree_child_link.pose_world))
        if self.type == 'revolute' and self.axis.lower_limit == 0 and self.axis.upper_limit == 0:
            ...
            dummylinknode = ET.SubElement(node, 'link', {'name': sdf2tfname(
                full_prefix + self.name + '::revolute_dummy_link')})
            childnode.attrib['link'] = dummylinknode.attrib['name']
            dummyjointnode = ET.SubElement(node, 'joint', {'name': sdf2tfname(
                full_prefix + self.name + '::revolute_dummy_joint')})
            ...
            self.axis2.add_urdf_elements(dummyjointnode, concatenate_matrices(
        ...
        self.axis.add_urdf_elements(jointnode, concatenate_matrices(
            inverse_matrix(self.pose_world), self.parent_model.pose_world))
...
class Axis(object):
    ...
    def from_tree(self, node):
        ...
        self.use_parent_model_frame = bool(
            get_tag(node, 'use_parent_model_frame'))
    ...
    def add_urdf_elements(self, node, modelCBTjoint):
        # SDF 1.4 axis is specified in model frame
        if (self.version <= 1.4) or (self.version >= 1.5 and self.use_parent_model_frame):
            rotation_modelCBTjoint = rotation_only(modelCBTjoint)
            xyz_joint = homogeneous_times_vector(
                rotation_modelCBTjoint, self.xyz)
            ...
            if xyz_norm != 0.0:
                ...
                print('Error calculating axis of joint %s for given xyz=%s' %
                      (self.joint.name, xyz_joint))
            ...
            xyz_joint = self.xyz
        axisnode = ET.SubElement(
            node, 'axis', {'xyz': array2string(rounded(xyz_joint))})
        limitnode = ET.SubElement(node, 'limit', {'lower': str(self.lower_limit), 'upper': str(
            self.upper_limit), 'effort': str(self.effort_limit), 'velocity': str(self.velocity_limit)})
...
class Inertial(object):
    ...
    def add_urdf_elements(self, node, link_pose):
        ...
        massnode = ET.SubElement(inertialnode, 'mass', {
                                 'value': str(self.mass)})
        # inertial reference frame is relative to the link reference frame in URDF and SDF
        pose2origin(inertialnode, self.pose)
...
class LinkPart(SpatialEntity):
    ...
    def from_tree(self, node):
        ...
        if node.tag != 'visual' and node.tag != 'collision':
            print(
                'Invalid node of type %s instead of visual or collision. Aborting.' % node.tag)
            return
        ...
        for gtype in self.gtypes:
            ...
            if typenode is not None:
                ...
                if gtype == 'box':
                    ...
                    self.geometry_data = {'radius': get_tag(
                        typenode, 'radius'), 'length': get_tag(typenode, 'length')}
                elif gtype == 'sphere':
                    self.geometry_data = {
                        'radius': get_tag(typenode, 'radius')}
                elif gtype == 'mesh':
                    self.geometry_data = {'uri': get_tag(typenode, 'uri'), 'scale': get_tag(
                        typenode, 'scale', '1.0 1.0 1.0')}
    ...
    def add_urdf_elements(self, node, prefix, link_pose, part_type):
        if not self.geometry_type:
            return
        partnode = ET.SubElement(node, part_type, {'name': sdf2tfname(
            prefix + '::' + self.name if prefix else self.name)})
        ...
        if self.geometry_type == 'box':
            ...
            cylindernode = ET.SubElement(geometrynode, 'cylinder', {
                                         'radius': self.geometry_data['radius'], 'length': self.geometry_data['length']})
        elif self.geometry_type == 'sphere':
            spherenode = ET.SubElement(geometrynode, 'sphere', {
            ...
            meshnode = ET.SubElement(geometrynode, 'mesh', {
                                     'filename': mesh_path, 'scale': self.geometry_data['scale']})
...
class Collision(LinkPart):
    ...
    def add_urdf_elements(self, node, prefix, link_pose):
        super(Collision, self).add_urdf_elements(
            node, prefix, link_pose, 'collision')
...
class Visual(LinkPart):
    ...
    def add_urdf_elements(self, node, prefix, link_pose):
        super(Visual, self).add_urdf_elements(
            node, prefix, link_pose, 'visual')
