...
from nightmare_pybullet.function_exec_manager import FuncExecManager
...
class pyBulletRosWrapper(object):
    ...
    def __init__(self):
        ...
        env_plugin = rospy.get_param('~environment', 'environment')
        plugin_import_prefix = rospy.get_param(
            '~plugin_import_prefix', 'nightmare_pybullet.plugins')
        self.environment = getattr(importlib.import_module(
            f'{plugin_import_prefix}.{env_plugin}'), 'Environment')(self.pb)
        ...
        for plugin in plugins:
            ...
            params_ = plugin.copy()
            rospy.loginfo(
                'loading plugin: {} class from {}'.format(class_, module_))
    ...
    def get_properties(self):
        ...
        for joint_index in range(0, self.pb.getNumJoints(self.robot)):
            ...
            if info[2] == self.pb.JOINT_REVOLUTE:
                ...
                rev_joint_index_name_dic[joint_index] = info[1].decode(
                    'utf-8')  # info[1] refers to joint name
            elif info[2] == self.pb.JOINT_FIXED:
                # insert key, value in dictionary (joint index, joint name)
                fixed_joint_index_name_dic[joint_index] = info[1].decode(
                    'utf-8')  # info[1] refers to joint name
            elif info[2] == self.pb.JOINT_PRISMATIC:
                prismatic_joint_index_name_dic[joint_index] = info[1].decode(
                    'utf-8')  # info[1] refers to joint name
    ...
    def init_pybullet_robot(self):
        ...
        if urdf_path is None:
            rospy.signal_shutdown(
                'mandatory param robot_urdf_path not set, will exit now')
        # test urdf file existance
        if not os.path.isfile(urdf_path):
            rospy.logerr(
                'param robot_urdf_path is set, but file does not exist : ' + urdf_path)
        ...
        robot_spawn_orientation = self.pb.getQuaternionFromEuler(
        ...
        # NOTE: does not currently work with effort controller, thats why is left as 0
    ...
    def start_nightmare_pybullet_wrapper_sequential(self):
        """
    ...
    def start_nightmare_pybullet_wrapper_parallel(self):
        """
    ...
    def start_nightmare_pybullet_wrapper(self):
        if rospy.get_param('~parallel_plugin_execution', True):
            self.start_nightmare_pybullet_wrapper_parallel()
        else:
            self.start_nightmare_pybullet_wrapper_sequential()


def main():
    """function called by nightmare_pybullet_node script"""
    rospy.init_node('nightmare_pybullet',
                    anonymous=False)  # node name gets overrided if launched by a launch file

    nightmare_pybullet_interface = pyBulletRosWrapper()
    nightmare_pybullet_interface.start_nightmare_pybullet_wrapper()
