...
class laserScanner:
    def __init__(self, pybullet, robot, **kargs):
        ...
        laser_frame_id = rospy.get_param('~laser/frame_id', None)
        if not laser_frame_id:
            rospy.logerr(
                'required parameter laser_frame_id not set, will exit now')
        ...
        if not (laser_frame_id in link_names_to_ids_dic):
            rospy.logerr('laser reference frame "{}" not found in URDF model, cannot continue'.format(
                laser_frame_id))
        ...
        # should be 512 beams but simulation becomes slow
        self.numRays = rospy.get_param('~laser/num_beams', 50)
        ...
        self.beam_visualisation = rospy.get_param(
            '~laser/beam_visualisation', False)
        ...
        self.pub_laser_scanner = rospy.Publisher(
    ...
    def transform_rays(self, laser_position, laser_orientation):
        ...
        laser_position = [laser_position[0],
                          laser_position[1], laser_position[2]]
        TFrayFrom = []
        TFrayTo = []
        ...
        for ray in self.rayFrom:
            position = np.dot(rotation_matrix, [
                              ray[0], ray[1], ray[2]]) + laser_position
    ...
    def execute(self):
        ...
        rayFrom, rayTo = self.transform_rays(
            laser_state[0], laser_state[1])  # position + orientation
